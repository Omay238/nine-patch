 ...
 * @param {number} mode - The mode of the NinePatch. 0 = stretch, 1 = repeat with stretch, 2 = repeat with cut.
...
class NinePatch {
    constructor(src, mode, patch1, patch2, patch3, patch4) {
        this.mode = mode;
        ...
        this.image = loadImage(src, (img) => {
            if (src.endsWith(".9.png")) {
                ...
                console.log(patch1, patch2, patch3, patch4)
    ...
    gen(w, h) {
        ...
        w = Math.max(this.patches[3].width + this.patches[7].width, w);
        h = Math.max(this.patches[1].height + this.patches[5].height, h);
